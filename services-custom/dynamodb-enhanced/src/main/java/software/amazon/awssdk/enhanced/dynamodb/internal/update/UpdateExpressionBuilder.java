    ...
    public static Expression generateUpdateExpression(Map<String, AttributeValue> attributeValuesToUpdate,
        ...
        attributeValuesToUpdate.forEach((key, value) -> {
            ...
            if (isAttributeAtomicCounter(counter)) {
                String expression = keyRef(key) + EQUALS + ifNotExists(key, counter.startValue().name()) +
                                    addValue(counter.delta().name());
                additionalAttributeValues.put(valueRef(counter.startValue().name()), counter.startValue().resolvedValue());
                additionalAttributeValues.put(valueRef(counter.delta().name()), counter.delta().resolvedValue());
